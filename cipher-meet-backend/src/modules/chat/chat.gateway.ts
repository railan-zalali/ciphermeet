...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(
    ...
    if (!conversation) {
      ...
      conversation = await this.conversationModel.findOne({
        participants: { $all: [senderId, payload.receiverId], $size: 2 },
      });
    }
  ...
  handleTypingStart(
    ...
    this.server
      .to(`user:${receiverId}`)
      .emit('typing:start', { userId: client.data.userId as string });
  }
  ...
  handleTypingStop(
    ...
    this.server
      .to(`user:${receiverId}`)
      .emit('typing:stop', { userId: client.data.userId as string });
  }
  ...
  async handleRead(
    ...
    this.server
      .to(`user:${senderId}`)
      .emit('message:read', { messageId, readAt: new Date() });
  }
  ...
  async handleReact(
    ...
    @MessageBody()
    {
      ...
      emoji,
      receiverId,
    }: { messageId: string; emoji: string; receiverId: string },
  ) {
    const userId = client.data.userId as string;
    await this.messageModel.updateOne(
      { _id: messageId },
    ...
    this.server
      .to(`user:${receiverId}`)
      .emit('message:reaction', { messageId, userId, emoji });
  }
}
